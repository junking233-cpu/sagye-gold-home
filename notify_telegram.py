...
prices.js를 직접 읽어서 카드 이미지를 만들고, 텔레그램 봇으로 전송합니다.
...
# ──────────────────────────────────────────────────
# prices.js에서 가격 직접 추출
# ──────────────────────────────────────────────────
def extract_prices_from_js():
    """prices.js 파일을 읽어서 가격 변수들을 추출"""
    with open(PRICES_JS, "r", encoding="utf-8") as f:
        content = f.read()

    def find_num(varname):
        m = re.search(rf"const\s+{varname}\s*=\s*(\d+)", content)
        if not m:
            raise ValueError(f"변수 못 찾음: {varname}")
        return int(m.group(1))

    return {
        "골드바_살때": find_num("골드바_살때"),
        "골드바_팔때": find_num("골드바_팔때"),
        "K18": find_num("K18"),
        "K14": find_num("K14"),
        "백금": find_num("백금"),
        "실버바_살때": find_num("실버바_살때"),
        "실버바_팔때": find_num("실버바_팔때"),
    }
...
# 카드 이미지 생성
# ──────────────────────────────────────────────────
def make_card(prices, date_str, time_str):
    items = [
        ("골드바 1돈",   won(prices["골드바_살때"]),  won(prices["골드바_팔때"])),
        ("18K",         "제품시세적용",                won(prices["K18"])),
        ("14K",         "제품시세적용",                won(prices["K14"])),
        ("백금",         "제품시세적용",                won(prices["백금"])),
        ("실버바 1kg",   won(prices["실버바_살때"]),   won(prices["실버바_팔때"])),
    ]
...
# 캡션
# ──────────────────────────────────────────────────
def make_caption(prices, date_str):
    return f"""🍀 사계절 금거래소 시세 안내
{date_str} 기준
...
🟡 골드바 1돈
   살 때 {won(prices['골드바_살때'])} / 팔 때 {won(prices['골드바_팔때'])}
...
🥈 실버바 1kg
   살 때 {won(prices['실버바_살때'])} / 팔 때 {won(prices['실버바_팔때'])}
━━━━━━━━━━━
...
def main():
    ...
    # prices.js에서 직접 가격 추출 (타이밍 문제 해결!)
    prices = extract_prices_from_js()
    print(f"💰 prices.js에서 직접 추출:")
    ...
    make_card(prices, date_str, time_str)
    caption = make_caption(prices, date_str)
